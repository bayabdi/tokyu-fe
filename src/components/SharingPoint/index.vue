  ...
  <div class="bg-white mt-7 pt-4 pb-4 px-4" v-show="contracts.length > 0">
    ...
    <div class="pb-7" v-for="(contract, i) in contracts" :key="contract.id">
      ...
      <div v-if="canSeeDetail"  class="text-center mt-10">
        <router-link :to="'/home/contract-information/Details/' + contract.id">
...
export default {
  ...
  methods: {
    getValidPoints (points) {
      ...
      if (validIndex > -1) {
        validPoints.push(sortedPoints[validIndex])
      }
      if (validIndex + 1 < sortedPoints.length) {
        validPoints.push(sortedPoints[validIndex + 1])
      }
